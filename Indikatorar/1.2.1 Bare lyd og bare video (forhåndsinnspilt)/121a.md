![alt text](https://www.difi.no/_style/design/difi3/img/difi-logo.png "Difi logo")  
Tilsyn for universell utforming av IKT - Arbeidsdokument
# 1.2.1a - Forhåndsinnspelt lyd har alternativ i form av tekst
**Status på indikatoren:** Under arbeid

**Dato for sist oppdatert:** 02.02.2017

Dette er 1 av 2 indikatorar som dekker suksesskriteriet i WCAG. Det må vere samsvar med alle indikatorane for å oppnå samsvar med kravet i WCAG.

## WCAG-krav
[1.2.1 Bare lyd og bare video (forhåndsinnspilt)](https://www.w3.org/TR/UNDERSTANDING-WCAG20/media-equiv-av-only-alt.html): For forhåndsinnspilt innhold som bare er lyd eller video gjelder følgende, bortsett fra når lyden eller videoen utgjør et mediealternativ til tekst og er tydelig merket som det.

Forhåndsinnspilt lyd (bare lyd): Det gis tilgang til et alternativ til tidsbaserte medier som presenterer informasjon som tilsvarer det forhåndsinnspilte lydinnholdet.
Forhåndsinnspilt video (bare video): Det gis tilgang til enten et alternativ til tidsbaserte medier eller et lydspor som presenterer informasjon som tilsvarer det forhåndsinnspilte videoinnholdet

## Oppsummert formål med kravet.
Forhåndsinnspelt lyd som ikkje er eit mediealternativ til tekst eller video, har alternativ i form av tekst. 

* Tekstalternativet er visuelt plassert nær lydklippet, eller kan nåast via ein mekanisme (lenke,knapp eller liknande) nær lydklippet.
* Tekstalternativet må vere koda som tekst.
* Tekstalternativet formidlar det same hovudinnhaldet som lydklippet, i lik rekkefølge.

## Kva testar vi?
* Forhåndsinnspelt lyd som ikkje er eit mediealternativ til tekst eller video

## Kva skal til for at kravet er oppfylt?
Forhåndsinnspelt lyd som ikkje er eit mediealternativ til tekst eller video, har alternativ i form av tekst. 
- Tekstalternativet er visuelt plassert nær lydklippet, eller kan nåast via ein mekanisme (lenke, knapp eller liknande) nær lydklippet.
- Tekstalternativet må vere koda som tekst.
- Tekstalternativet formidlar det same hovudinnhaldet som lydklippet, i lik rekkefølge. 
- MERK: Vi krev ikkje eksakt attgjevinng av innhaldet, men at alt vesentleg innhald er teke med i rett rekkefølge.

## Korleis testar vi?
Testmetoden er manuell, og vi vurderer  følgjande:
* Er lydklippet forhåndsinnspelt eller direkte?
* Er lydklippet mediealternativ til tekst, og tydeleg merka som dette? 
* Finst det tekst eller lenke eller liknande mekanisme som leder til tekst som står fram som alternativ til lydklippet?
* Ligg teksten visuelt plassert nær det aktuelle lydklippet?
* Kor mange tab-steg er det frå lydklippet til lenkja/knappen? 
* Er teksten koda som tekst? 
Deretter samanliknar vi teksten med innhaldet i lydfila. Vi registrerer om informasjonen som blir formidla i lydklippet er i samsvar med informasjon som blir formidla i teksten.


### 1. Førebuing
Første del av testprosedyren. Førebuinga går ut på å gjere seg klar til test.

<table>
  <tr>
    <th scope="col">Steg</th>
    <th scope="col">Instruksjon</th>
    <th scope="col">Hjelp</th>
    <th scope="col">Dataregistering</th>
  </tr>
  <tr>
    <th scope="row">1.1</th>
    <td>Opne nettlesaren.</td>
    <td></td>
    <td></td>
  </tr>
  <tr>
    <th scope="row">1.2</th>
    <td>Opne førehandsdefinert testside eller gjennomfør eit manuelt søk etter lydklipp på nettstaden. <br>
      <br>
      Dersom du ikkje har funne lydklipp innan få minuttar, kan du avbryte.<br></td>
    <td>Aktuelle metodar kan vere:<br>
      <br>
      - Sjå på sidene om det finst ein playknapp eller liknande <br>
      - Bruk internt søk på nettstaden<br>
      - Bruk Google (Du kan bruke "site:urlnettstad søkeord" for å    berre søke på ein bestemt nettstad).<br>
      <br>
      Aktuelle søkeomgrep kan være: "Podcast"/"Podkast", "Lydklipp", "Lyd*", "Lydfil",  "opptak", "Hør her", "Høyr her",    "Lytt","mp3"</td>
    <td></td>
  </tr>
  <tr>
    <th scope="row">1.3</th>
    <td><p>Har nettstaden lydklipp?</p>
    <p><b>Viss Nei, gå til punkt 3.10 for å skrive inn kommentar. Nettstaden er ikkje aktuell for test.</b></p></td>
    <td>&nbsp;</td>
    <td><ul>
      <li>Ja</li>
      <li>Nei</li>
    </ul></td>
  </tr>
  <tr>
    <th scope="row">1.4</th>
    <td>Registrer kva nettlesar du testar i.</td>
    <td></td>
    <td>Namn på nettlesar</td>
  </tr>
  <tr>
    <th scope="row">1.5</th>
    <td>Registrer kva versjon av nettlesaren du testar i.</td>
    <td>Denne finn du ofte under "om" eller i innstillingar for nettlesaren. Ta med heile versjonsnummeret på nettlesaren.</td>
    <td>Versjon nettlesar</td>
  </tr>
</table>

### 2. For kvart lydklipp
Andre del av testprosedyren. Kvalifiseringa går ut på å fange opp om det finst innhald som skal testast.

<table>
  <tbody>
    <tr>
      <th scope="col">Steg</th>
      <th scope="col">Instruksjon</th>
      <th scope="col">Hjelp</th>
      <th scope="col">Dataregistrering</th>
      <th scope="col">Kilde</th>
    </tr>
    <tr>
      	<th scope="row">2.1</th>
		<td>Er lydklippet forhåndsinnspelt??<br><br>
<b>Viss Nei, gå til punkt 3.10 for å skrive inn kommentar. Du har ein ikkje-forekomst. </b></td>
      <td>Aktuelle metodar for å finne ut dette er:
<br>- Prøv å starte lydklippet og sjå om avspelinga startar frå byrjinga av fila og om det finst informasjon om varigheiten på klippet.
<br>-  Sjå etter informasjon om når klippet er publisert.
<br>- Sjå etter informasjon som tilseier at klippet er live/direkte. 
Dette kan for eksempel vere: Rødt felt med teksten "live" eller at det ikkje er mogeleg å spole i klippet.</td>
    <td>
    <ul>
    <li>Ja</li>
    <li>Nei</li>
    </ul></td>
    <td></td>
    </tr>
       <tr>
      	<th scope="row">2.2</th>
		<td>Er lydklippet eit mediealternativ til tekst eller mediealternativ til video uten lyd og tydeleg merka som det?<br><br>
<b>Viss Nei, gå til punkt 3.10 for å skrive inn kommentar. Du har ein ikkje-forekomst. </b></td>
      <td>Undersøk om det i tilknytning til lydklippet finst ein tekst som visuelt står fram som hovudformatet for formidling av informasjonen. 
<br><br>
Indikasjonar på at lydklippet er eit mediealternativ til tekst kan vere:
<br>- Ikon (for eksempel høgtalar, noteikon) som indikerer at lydklippet er eit alternativ til ein nærliggande tekst eller video utan lyd.
<br>- Lenke med lenketekst der det går fram at det er eit lydklipp (for eksempel "lytt til teksten", "les opp teksten").
<br>-  Eit mediealternativ til tekst eller video utan lyd er tydeleg merka når det er presentert i direkte nærleik til ein tekst/video og det ikkje er tvil om at det er et alternativ til den aktuelle teksten/videoen.</td>
    <td>
    <ul>
    <li>Ja</li>
    <li>Nei</li>
    </ul></td>
    <td></td>
    </tr>
  </tbody>
</table>

### 3. For kvart forhåndsinnspelte lydklipp som ikkje er eit mediealternativ til tekst
Tredje del av testprosedyren. Testen går ut på å sjekke ulike måtar å oppfylle kravet på.

<table>
  <tr>
 <th scope="col">Steg</th>
 <th scope="col">Instruksjon</th>
 <th scope="col">Hjelp</th>
 <th scope="col">Dataregistering</th>
 <th scope="col">Kilde</th>
  </tr>
  <tr>
 <th scope="row">3.1</th>
 <td>Oppgi url for sida lydklippet ligg på.</td>
 <td>Skriv inn url på ei slik form at vi kan nå aktuell nettside ved å klikke på lenka</td>
 <td>URL</td>
 <td></td>
  </tr>
  <tr>
 <th scope="row">3.2</th>
 <td>Skriv inn tittel på lydklippet. </td>
 <td>Legg inn tittel på lydklippet eller stikkord om innhaldet, dersom tittel manglar.<br>
   <br>
   Merk at du må registrere nok informasjon til at lydklippet kan identifiserast.</td>
 <td>Tittel på lydklipp</td>
 <td></td>
  </tr>
  <tr>
 <td colspan="5">I det følgjande skal du vurdere om det finst tekstalternativ visuelt plassert nær lydklippet/avspelaren</td>
 </tr>
  <tr>
 <th>3.3</th>
 <td>Finst det ein tekst visuelt plassert nær lydklippet/avspelaren som omtaler klippet?<br>
   <br>
    <b>Viss Nei, kan du kan du hoppe til 3.6. </b></td>
 <td>I dette steget skal du ikkje ta stilling til kvaliteten på teksten. Det er nok at teksten ligg nær klippet, den kan for eksempel ligge over eller under lydklippet.<br>
   <br>
   Du skal ikkje måtte scrolle vekk frå lydklippet for å finne teksten. (Du kan scrolle på sida, men lydklippet skal fortsatt vere synleg i nettlesaren).<br>
   <br>
   Merk at det er ok at du må scrolle for å lese gjennom teksten.</td>
 <td><ul>
   <li>Ja</li>
   <li>Nei</li>
 </ul></td>
 <td></td>
  </tr>
  <tr>
 <th>3.4</th>
 <td>Er tekstalternativet koda som tekst?<br>
   <br>
    <b>Viss Nei, kan du kan du hoppe til 3.6. </b></td>
 <td>Dette kan undersøkast på fleire måtar:<br>
   <br>
   Alternativ 1:<br>
   - Sjå om du får til å markere teksten med mus eller tastatur. Dette indikerer at testen er koda som tekst og ikkje er eit bilete av tekst.<br>
   <br>
   Alternativ 2:<br>
   - Opne kodeverktøyet i nettlesaren.<br>
   - Bruk  "select " funksjonen i verktøyet og markere et område i tekstalternativet.<br>
   - Sjekk at tekstalternativet er koda som tekst, ved å sjå om du finn att teksten i koden.</td>
 <td><ul>
   <li>Ja</li>
   <li>Nei</li>
 </ul></td>
 <td>G158<br>
   F30</td>
  </tr>
  <tr>
 <th>3.5</th>
 <td>Er informasjonen/innhaldet som blir formidla i lydklippet i samsvar med informasjon som blir formidla i teksten?  <br>
   <br>
    <b>Viss Ja, gå til punkt 3.10 for å skrive inn kommentar. </b> </td>
 <td>Samanlikn tekstalternativet med innhaldet i lydklippet:<br>
   <br>
   - Gå gjennom dei første minuttane av lydklippet.<br>
   - Vurder om lydklipp og tekstalternativ formidlar det same hovudinnhaldet i lik rekkefølge. <br>
   - MERK: Vi krev ikkje eksakt attgjevinng av innhaldet, men at alt vesentleg innhald er teke med i rett rekkefølge.<br>
   <br>
   Dersom du allereie i starten av lydklippet avdekkar at det ikkje er samsvar mellom lydklipp og tekstalternativ, kan du stoppe gjennomgangen.</td>
 <td><ul>
   <li>Ja</li>
   <li>Nei</li>
 </ul></td>
 <td>G158<br>
   F30<br>
   F67</td>
  </tr>
  <tr>
 <td colspan="5">I det følgjande skal du vurdere om du når eit tekstalternativ via ein mekanisme i form av ei lenke, ein knapp eller liknande</td>
  </tr>
  <tr>
 <th>3.6</th>
 <td>Finst det ein mekanisme (lenke, knapp eller liknande) nær lydklippet som tilseier at du vert leia til eit tekstalternativ til lydklippet?<br>
   <br>
    <b>Viss Nei, gå til punkt 3.10 for å skrive inn kommentar. </b></td>
 <td>Du skal ikkje måtte scrolle vekk frå videoklippet for å sjå at det finst ein mekanisme.<br>
   <br>
   Merk at det er ok at du må scrolle for å lese gjennom tekstalternativet. (Du kan scrolle på sida, men videoklippet skal fortsatt vere synleg i nettlesaren).</td>
 <td><ul>
   <li>Ja</li>
   <li>Nei</li>
 </ul></td>
 <td></td>
  </tr>
  <tr>
 <th>3.7</th>
 <td>Start tastaturnavigeringa frå klippet. Kor mange tab-steg treng du for å nå mekanismen (lenka/knappen eller liknande)?<br>
   <br>
   (Merk at antall tab-steg berre blir nytta til kartlegging og ikkje inngår i vudering av samsvar/avvik)</td>
 <td>Merk at mekanismen (knappen/lenka eller liknande) kan anten ligge før eller etter i tab-rekkefølgja og at begge plasseringar er godkjende.<br>
   <br>
   Registrer ved hjelp av svaralternativa kor mange tabsteg du treng for å nå mekanismen som leiar til tekstalternativet. Stopp om du treng meir enn 10 tab-steg, ref. svaralternativa. </td>
 <td><ul>
   <li>1-5 tabsteg</li>
   <li>6-10 tabsteg</li>
   <li>Fleire enn 10 tabsteg</li>
   <li>Når ikkje mekanisme med tastatur</li>
   <li>Ikkje mogleg å verifisere</li>
 </ul></td>
 <td></td>
  </tr>
  <tr>
 <th>3.8</th>
 <td>Er tekstalternativet koda som tekst?<br>
    <b><br>
   Viss Nei, gå til punkt 3.10 for å skrive inn kommentar. </b> </td>
 <td>Dette kan undersøkast på fleire måtar:<br>
   <br>
   Alternativ 1:<br>
   - Sjå om du får til å markere teksten med mus eller tastatur. Dette indikerer at testen er koda som tekst og ikkje er eit bilete av tekst.<br>
   <br>
   Alternativ 2:<br>
   - Opne kodeverktøyet i nettlesaren.<br>
   - Bruk  "select " funksjonen i verktøyet og markere et område i tekstalternativet.<br>
   - Sjekk at tekstalternativet er koda som tekst, ved å sjå om du finn att teksten i koden.</td>
 <td><ul>
   <li>Ja</li>
   <li>Nei</li>
 </ul></td>
 <td>G158</td>
  </tr>
  <tr>
 <th>3.9</th>
 <td>Er informasjonen/innhaldet som blir formidla i lydklippet i samsvar med informasjon som blir formidla i teksten?</td>
 <td>Samanlikn tekstalternativet med innhaldet i lydklippet:<br>
   <br>
   - Gå gjennom dei første minuttane av lydklippet.<br>
   - Vurder om lydklipp og tekstalternativ formidlar det same hovudinnhaldet i lik rekkefølge. <br>
   - MERK: Vi krev ikkje eksakt attgjevinng av innhaldet, men at alt vesentleg innhald er teke med i rett rekkefølge.<br>
   <br>
   Dersom du allereie i starten av lydklippet avdekkar at det ikkje er samsvar mellom lydklipp og tekstalternativ, kan du stoppe gjennomgangen.</td>
 <td><ul>
   <li>Ja</li>
   <li>Nei</li>
 </ul></td>
 <td>G158<br>
   F30<br>
   F67</td>
  </tr>
  <tr>
 <th>3.10</th>
 <td>Eventuelt: <br>
   Legg til kommentar som utdjupar testresultat eller liknande.<br>
   <br>
    <b>Sjå utfallstabellen for å finne ditt resultat. </b></td>
 <td>Legg inn kommentar i dataregistreringsfeltet</td>
 <td>Tekst</td>
 <td></td>
  </tr>
</table>

## Utfall
Oversikt over kva registreringar i testprosedyren som gir kva utfall på indikatoren.

Eit utfall er eit moleg resultat etter ulike kombinasjonar av svar på stega i testprosedyren. Utfallet gir ei standardisert og kort oppsummering av observasjonen og om det er samsvar med fasit.

<table>
  <tr>
     <th scope="col">Utfall</th>
     <th scope="col">Samsvar</th>
     <th scope="col">Vurdering</th>
     <th scope="col">Skala</th>
     <th scope="col">3.3</th>
     <th scope="col">3.4</th>
     <th scope="col">3.5</th>
     <th scope="col">3.6</th>
     <th scope="col">3.7</th>
     <th scope="col">3.8</th>
     <th scope="col">3.9</th>
  </tr>
  <tr>
    <th scope ="row">Lydklipp    uten tekstalternativ nær klippet og utan mekanisme til anna tekstalternativ.</th>
    <td>Brudd</td>
    <td>Utilgjengeleg</td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td></td>
  </tr>
  <tr>
    <th scope="row">Lydklipp uten tekstalternativ nær klippet og tekstalternativ via    mekanisme som ikkje er koda som tekst.</th>
    <td>Brudd</td>
    <td>Utilgjengeleg</td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
  </tr>
  <tr>
    <th scope="row">Lydklipp uten tekstalternativ nær klippet og tekstalternativ via    mekanisme som ikkje gir same bodskap.</th>
    <td>Brudd</td>
    <td>Problem å bruke</td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>
  </tr>
  <tr>
    <th scope="row">Tekstalternativ nær klippet som ikkje er koda som tekst og utan    mekanisme til anna tekstalternativ.</th>
    <td>Brudd</td>
    <td>Utilgjengeleg</td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>    
    <td></td>  
  </tr>
  <tr>
    <th scope="row">Tekstalternativ nær klippet som ikkje er koda som tekst og    tekstalternativ via mekanisme som ikkje er koda som tekst.</th>
    <td>Brudd</td>
    <td>Utilgjengeleg</td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>    
    <td></td>  
  </tr>
  <tr>
    <th scope="row">Tekstalternativ nær klippet som ikkje er koda som tekst og    tekstalternativ via mekanisme som ikkje gir same bodskap.</th>
    <td>Brudd</td>
    <td>Problem å bruke</td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>
  </tr>
  <tr>
    <th scope="row">Tekstalternativ nær klippet som ikkje gir same bodskap og utan    mekanisme til anna tekstalternativ.</th>
    <td>Brudd</td>
    <td>Problem å bruke</td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td></td>
  </tr>
  <tr>
    <th scope="row">Tekstalternativ nær klippet som ikkje gir same bodskap og    tekstalternativ via mekanisme som ikkje er koda som tekst.</th>
    <td>Brudd</td>
    <td>Problem å bruke</td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
  </tr>
  <tr>
    <th scope="row">Tekstalternativ nær klippet som ikkje gir same bodskap og    tekstalternativ via mekanisme som ikkje gir same bodskap.</th>
    <td>Brudd</td>
    <td>Problem å bruke</td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>
    <td></td>
    <td></td>
    <td></td>
    <td>nei</td>
  </tr>
  <tr>
    <th scope="row">Lydklippet har eit tekstalternativ som kan nåast, koda som tekst og som gir same bodskap som lydklippet. </th>
    <td>Samsvar</td>
    <td>Samsvar</td>
    <td></td>
    <td>ja</td>
    <td>ja</td>
    <td>ja</td>
    <td></td>
    <td></td>
    <td></td>
    <td></td>
  </tr>
  <tr>
    <th scope="row">Lydklippet har eit tekstalternativ som kan nåast, koda som tekst og som gir same bodskap som lydklippet. </th>
    <td>Samsvar</td>
    <td>Samsvar</td>
    <td></td>
    <td></td>
    <td></td>
    <td></td>
    <td>ja</td>
    <td></td>
    <td>ja</td>
    <td>ja</td>
  </tr>
</table>

## Teknikkar
* [G158: Providing an alternative for time-based media for audio-only content](http://www.w3.org/TR/2016/NOTE-WCAG20-TECHS-20160317/G158)
* [F30: Failure of Success Criterion 1.1.1 and 1.2.1 due to using text alternatives that are not alternatives (e.g., filenames or placeholder text)](http://www.w3.org/TR/2016/NOTE-WCAG20-TECHS-20160317/F30)
* [F67: Failure of Success Criterion 1.1.1 and 1.2.1 due to providing long descriptions for non-text content that does not serve the same purpose or does not present the same information](http://www.w3.org/TR/2016/NOTE-WCAG20-TECHS-20160317/F67)

## Vedlegg
Indikator 1.2.1 a i Excel-format (Lenke kjem)
